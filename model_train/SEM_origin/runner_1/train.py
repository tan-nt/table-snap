...
def train(cfg, epoch, num_epochs, dataloader, model, optimizer, scheduler, time_counter, synchronizer=None):
    ...
    for it, data_batch in enumerate(dataloader):
        ...
        try:
            ...
            loss_terms = [val for key, val in result_info.items() if 'loss' in key and isinstance(val, torch.Tensor)]
            if loss_terms:
                loss = torch.stack(loss_terms).sum()
                loss.backward()
            else:
                print("[Warning] No loss terms found in result_info. Skipping backward.")
