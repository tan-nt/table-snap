...
model_paths = {
    ...
    'resnet34': './pretrain_model/resnet_34.pth',
...
def _resnet(
    ...
    **kwargs: Any
    ...
    if pretrained:
        checkpoint = torch.load(model_paths[arch], map_location='cpu', weights_only=False)
...
def build_backbone(arch, pretrained=True, norm_layer=nn.BatchNorm2d):
    """
    Build backbone network. Supports ResNet and EfficientNet.
    Returns a feature extractor returning intermediate feature maps.
    """
    if arch.startswith('res'):
        ...
        if arch not in arch_map:
            raise ValueError('Unknown ResNet arch: %s' % arch)
        backbone = arch_map[arch](pretrained=pretrained, norm_layer=norm_layer)
        # Remove classification layers
        modules = list(backbone.children())[:-2]  # remove avgpool + fc
        return nn.Sequential(*modules)

    elif arch.startswith('efficientnet'):
        # e.g., efficientnet_b0, efficientnet_b3, efficientnet_b4, etc.
        try:
            backbone = timm.create_model(arch, pretrained=pretrained, features_only=True)
        except:
            raise ValueError('Invalid EfficientNet arch: %s' % arch)
        return backbone

    else:
        raise ValueError('Unsupported backbone arch: %s' % arch)