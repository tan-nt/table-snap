...
def extend_segments(row_segments, rows_es, col_segments, cols_es, cells_spans, layouts, divide_labels):
    ...
    for batch_idx in range(batch_size):
        ...
        # ✅ Fix: safely append divide_label
        divide_label_val = divide_labels[batch_idx].item()
        if divide_label_val in row_segments_idx:
            ext_divide_labels.append(row_segments_idx.index(divide_label_val))
        else:
            # Fallback to closest or default
            ext_divide_labels.append(0)  # or -1, or len(row_segments_idx) // 2
            print(f"[extend_segments] Warning: divide_label {divide_label_val} not found in row_segments_idx")
        ...
        for cell_idx, cell_span in enumerate(cells_spans_pi):
            ...
            try:
                ...
                print(f"[extend_segments] Skipping invalid cell_span {cell_span}: {ve}")
    ...
    return (
        ...
        ext_col_segments,
        ext_cells_spans,
        aligned_layouts(ext_layouts, layouts),
        torch.tensor(ext_divide_labels).to(divide_labels.device)
    )
...
def aligned_layouts(layouts_list, layouts):
    ...
    max_row_nums = max([l.shape[0] for l in layouts_list] + [1])  # ensure at least 1
    max_col_nums = max([l.shape[1] for l in layouts_list] + [1])  # ensure at least 1

    aligned_layouts = list()
    for batch_idx in range(batch_size):
        layout_np = layouts_list[batch_idx]

        if layout_np.shape[0] == 0 or layout_np.shape[1] == 0:
            print(f"[aligned_layouts] Warning: skipping empty layout with shape {layout_np.shape}")
            layout_np = np.full((1, 1), -1, dtype=layout_np.dtype)

        num_rows_pi = layout_np.shape[0]
        num_cols_pi = layout_np.shape[1]

        layouts_pi = torch.from_numpy(layout_np).to(dtype=dtype, device=device)
