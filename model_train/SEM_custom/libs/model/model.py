...
class Model(nn.Module):
    ...
    def forward(self, images, images_size, cls_labels=None, labels_mask=None, layouts=None, rows_fg_spans=None,
        ...
        if isinstance(feats, (list, tuple)) and len(feats) > 4:
            feats = feats[-4:]  # EfficientNet typically returns more than 4 stages


        feats = self.fpn(feats)
        print("\n>> Feature map channels:", [f.shape[1] for f in feats])

        for i, f in enumerate(feats):
            print(f"[DEBUG] FPN Output P{i+2}:", f.shape)
        ...
        rs_result_info = {'row_' + key: val for key, val in rs_result_info.items()}
        rs_ext_info = {'row_' + key: val for key, val in rs_ext_info.items()}
        ...
        cs_result_info = {'col_' + key: val for key, val in cs_result_info.items()}
        cs_ext_info = {'col_' + key: val for key, val in cs_ext_info.items()}
        result_info.update(cs_result_info)
        ext_info.update(cs_ext_info)

        if self.training:
            try:
                row_segments, col_segments, cells_spans, layouts, divide_labels = extend_segments(
                    row_segments, rs_ext_info['row_ext_segments'],
                    col_segments, cs_ext_info['col_ext_segments'],
                    cells_spans, layouts, divide_labels)
            except Exception as e:
                print("[Error] extend_segments failed:", e)
                raise e
        ...
        print("[DEBUG] feat_maps:", feat_maps.shape)
        print("[DEBUG] feats_masks:", feats_masks.shape)
        print("[DEBUG] layouts:", layouts.shape)
        print("[DEBUG] cls_labels:", cls_labels.shape if cls_labels is not None else None)
        print("[DEBUG] labels_mask:", labels_mask.shape if labels_mask is not None else None)

        if self.training:
            if feat_maps.shape[-2:] != layouts.shape[-2:]:
                print(f"[Error] Feature map shape: {feat_maps.shape[-2:]}, Layout shape: {layouts.shape[-2:]}")
                raise ValueError("feat_maps is not the same with layouts")
        ...
        if not self.training:
            if de_preds.shape[0] != 1:
                raise ValueError("Batch size should be 1 during evaluation")
