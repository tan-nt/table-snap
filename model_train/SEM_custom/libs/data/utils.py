...
def cal_row_span(table, cells_span, cells_bbox, height):
    ...
    for row_idx in range(layout.shape[0]):
        ...
        y2s = list()
        for cell_id in row:
            # ✅ Skip invalid cell_id
            if cell_id < 0 or cell_id >= len(cells_span) or cell_id >= len(cells_bbox):
                continue
        ...
        if (len(y1s) > 0) and (len(y2s) > 0):
            ...
            continue
            # raise InvalidFormat()  # or just: rows_span.append([0, 0])
...
def cal_col_span(table, cells_span, cells_bbox, width):
    ...
    for col_idx in range(layout.shape[1]):
        ...
        for cell_id in col:
            if cell_id < 0 or cell_id >= len(cells_span):
                continue  # skip invalid cell ID

            if cell_id < 0 or cell_id >= len(cells_bbox):
                continue  # skip invalid cell ID
            ...
            if cell_span[0] == col_idx:
                x1s.append(cell_bbox[0])
            if cell_span[2] == col_idx:
        ...
        if x1s and x2s:
            ...
            raise InvalidFormat(f"Column {col_idx} has no valid bounding boxes.")
