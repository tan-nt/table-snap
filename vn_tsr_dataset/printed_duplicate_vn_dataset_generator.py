...
if __name__ == '__main__':
    # based_image_files = [{
    #     'img_file_name': 'vn_tsr_dataset/digital/digital_table_000001/img/digital_table_000001.jpg',
    ...
    for i in range(1, 28):
        ...
        if not os.path.exists(f'vn_tsr_dataset/printed/printed_table_{six_digits_str}'):
            print(f'vn_tsr_dataset/printed/printed_table_{six_digits_str} not exist')
            break

        based_image_files.append({
            'img_file_name': f'vn_tsr_dataset/printed/printed_table_{six_digits_str}/img/printed_table_{six_digits_str}.png',
            'annotation_file_name': f'vn_tsr_dataset/printed/printed_table_{six_digits_str}/annotation/content.html',
            'annotation_structure_file_name': f'vn_tsr_dataset/printed/printed_table_{six_digits_str}/annotation/structure.json',
        })

    if len(based_image_files) == 0:
        print('No based image files found')
        exit()

    print(f'Found {len(based_image_files)} based image files, data={based_image_files}')

    from albumentations import PadIfNeeded

    start_printed_folder_number = 28
    end_printed_folder_number = 801
    for i in range(start_printed_folder_number, end_printed_folder_number):
        six_digits_str = str(i).zfill(6)
        if os.path.exists(f'vn_tsr_dataset/printed/printed_table_{six_digits_str}'):
            shutil.rmtree(f'vn_tsr_dataset/printed/printed_table_{six_digits_str}')


        print(f'vn_tsr_dataset/printed/printed_table_{six_digits_str} not exist, start creating a stimulated image')
        os.makedirs(f'vn_tsr_dataset/printed/printed_table_{six_digits_str}')
        os.makedirs(f'vn_tsr_dataset/printed/printed_table_{six_digits_str}/img')
        os.makedirs(f'vn_tsr_dataset/printed/printed_table_{six_digits_str}/annotation')
