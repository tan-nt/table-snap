...
def cal_segments(span_cls_probs, spans, scale):
    segments = []
    for span in spans:
        if span_cls_probs.numel() == 0:
            continue  # or append a fallback segment
...
def cal_ext_segments(cls_probs, lengths, bg_spans, scale=1, threshold=0.5):
    ...
    for batch_idx in range(batch_size):
        ...
        # 🛡️ Skip empty length or empty bg_spans
        if length == 0 or len(bg_spans[batch_idx]) == 0:
            ext_segments.append([])
            continue

        # 🧠 Safe slicing
        cls_probs_slice = cls_probs[batch_idx, :length]

        ext_segments_pi = cal_segments(cls_probs_slice, bg_spans[batch_idx], scale)

        # 🧼 Only keep segments that pass confidence threshold
        ext_segments_pi = [
            segment for segment in ext_segments_pi
            if segment < len(cls_probs[batch_idx]) and cls_probs[batch_idx, segment] > threshold
        ]
...
class SegmentPredictor(nn.Module):
    ...
    def forward(self, feats, images_size, fg_spans=None, bg_spans=None):
        ...
        if self.training:
            ...
            if targets.shape[2:] != segments_logit.shape[2:]:
                targets = F.interpolate(targets.float(), size=segments_logit.shape[2:], mode='bilinear', align_corners=False)
            ...
            # mask shape must match targets
            masks_resized = F.interpolate(masks[:, None, :, :].float(), size=segments_logit.shape[2:], mode='nearest')
            segments_loss = (segments_loss * masks_resized).sum() / targets.sum()
