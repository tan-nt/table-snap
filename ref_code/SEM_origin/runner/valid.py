...
def valid(cfg, dataloader, model):
    ...
    val_log_file = os.path.join("logs", "valid_metrics.csv")
    if not os.path.exists(val_log_file):
        with open(val_log_file, "w", newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["epoch", "precision", "recall", "f1", "time_elapsed_s"])

    start_time = time.time()

    for it, data_batch in enumerate(tqdm.tqdm(dataloader)):
        if it >= cfg.max_valid_iters:
            logger.info("Stop early due to max_valid_iters=%d" % cfg.max_valid_iters)
            break
        ...
        logger.info(f"[Valid][Iter {it+1:04d}/{len(dataloader):04d}] Time Elapsed: {iter_elapsed:.2f}s")
    ...
    logger.info('[Valid] Total Type Metric: Precision: %.4f, Recall: %.4f, F1-Score: %.4f, Time: %.2fs' %
                (P, R, F1, total_elapsed))

    # Save to CSV
    with open(val_log_file, "a", newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            getattr(cfg, "current_epoch", 1),  # Optional: add epoch if available
            round(P, 4),
            round(R, 4),
            round(F1, 4),
