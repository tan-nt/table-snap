...
def train(cfg, epoch, num_epochs, dataloader, model, optimizer, scheduler, time_counter, synchronizer=None):
    ...
    for it, data_batch in enumerate(dataloader):
        ...
        try:
            ...
            logger.info('train raise exception, error: %s' % e)
        ...
        if it % cfg.log_sep == 0:
            ...
            logger.info(
                '[Train][Epoch %03d/%03d Iter %04d/%04d][Memory: %.0f ][Mean LR: %.6f ][Left: %s][Time Elapsed: %.2fs] %s' %
...
def valid(cfg, dataloader, model):
    ...
    for it, data_batch in enumerate(tqdm.tqdm(dataloader)):
        if cfg.max_valid_iters is not None and it >= cfg.max_valid_iters:
            print(f"[valid] Early stopping at iteration {it}, max_valid_iters={cfg.max_valid_iters}")
            break
        ids = data_batch['ids']
        images_size = data_batch['images_size']
        images = data_batch['images'].to(cfg.device)
        ...
        label_relations = []
        for table in tables:
            label_path = table.get('label_path')
            if label_path is None:
                print("[Warning] Missing 'label_path' in table:", table)
                continue  # hoặc xử lý fallback nếu cần
            # else:
            #     print('label_path=', label_path)
            label_path = os.path.join(cfg.valid_data_dir, label_path)
